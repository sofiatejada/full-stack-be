...
export default class Seen {
  ...
  static async insert({ sprite, name, primaryType, secondaryType }) {
    const { rows } = await pool.query('INSERT INTO seen (sprite, name, primary_type, secondary_type) VALUES ($1, $2, $3, $4) RETURNING *', [sprite, name, primaryType, secondaryType]);
  ...
  static async deleteById(id) {
    const { rows } = await pool.query('DELETE FROM seen WHERE id=$1 RETURNING *', [id]);
